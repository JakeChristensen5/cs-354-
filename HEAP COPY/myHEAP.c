...
*/
    ...
    while(storeSize_Status != 1 && storeSize_Status != 0){
        ...
        if(storeSize_Status == newSize){ //if the store block is == to the requested payload 

            //change
            temp = (blockHeader*)(store); //set best == store 
          ...
          if(storeSize_Status != NULL){
            ...
            if((P_BIT[0] == A_BIT_MASK && P_BIT[0] != 0) || storeSize_Status != 1 || newSize == 0){
                ...
                if(storeSize_Status != 1){
                  ...
                  //change
                next = (blockHeader*)((void*)store + adjustedPrevSize);
                }
            }
                ...
                if(!(next -> size_status == 1)){ 
                    ...
                    return (void*)(temp + 1); } } //return best
            }
        }
...
while(1 && ptr > 0 && foot -> size_status != 1){ //iterate through heap
...
if(!check[0] == NULL){
    ...
    if(updatedPtr < newHeap || updatedPtr > newHeap + alloc_size - 8) {
        return -1;
    }
   ...
   /*Update header(s) and foot as needed */
    ...
    while(1 && P_BIT){ //iterate to find match

    if(storeSize_Status != 1){
        
    /*Set the Next Block*/
    next = ((void*)checkStore[0] + storeSize_Status); 

    //if the next block reaches the end of the heap than return NULL
    if(next -> size_status == 1){
        return NULL;
    }

    //else if hasn't set its block header
...
int coalesce() {
    ...
    futureSize_Status = next->size_status;
...
if(!(futureSize_Status == 1)){
    if(next != NULL){
        next = (mergeSize_Status + (void*)merge);
    } 
...
if(!(futureSize_Status == 1) && totalSize != 0){
	if (futureSize_Status & P_BIT_MASK)
	    futureSize_Status -= 2;
        
    if (futureSize_Status & 2)
	    totalSize[0] += mergeSize_Status + futureSize_Status;
    }
    else {
        totalSize[0] = 0;
        return NULL;
...
while(1 && !(newFoot -> size_status == 1) && !(merge -> size_status == 1)){ //as long as foot or merge don't reach end of heap

    newFoot = mergeSize_Status + futureSize_Status + (void*)merge - MASK;

	if (A[0] != 1 && A[0] != 1) {

        merge->size_status = mergeSize_Status + futureSize_Status;
	    newFoot->size_status = mergeSize_Status + futureSize_Status;
